if (!start && oPlayer.currentState == "ground")
{
    start = 1;
    ...
    with (oPlayer)
    {
        var _targetCameraArea = instance_place(x, y, oCameraActivateArea);
        
        with (_targetCameraArea)
            lerpRateV = 0.2;
    }
}
...
if (start)
{
    ...
    function drawGuideText()
    {
        ...
        scribble(_str).starting_format(global.defaultFont, make_color_rgb(255, 255, 255)).blend(make_color_rgb(255, 255, 255), guide.alpha).msdf_border(make_color_rgb(46, 50, 59), 3).wrap(280, -1, locIsAsian()).transform(0.4, 0.4, 0).align(1, 1).draw(guide.drawx, guide.drawy + 32 + 8);
    }
