if (recJumpTimes > global.jumpTimes && recGrounded)
{
    mask_index = mask_nomask;
    myAlarm4.setTimer(10);
}

recJumpTimes = global.jumpTimes;
recGrounded = instance_exists(oPlayer) && oPlayer.currentState == "ground";
...
if (set && !stop)
{
    if (!pressed)
    {
        dampening = 0.25;
        tension = dampening / 3;
        xdisplacement = origx - x;
        ydisplacement = origy - y;
