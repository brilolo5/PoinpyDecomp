function input_default_gamepad_swap_ab(arg0)
{
    with (global.__input_default_player)
    {
        var _source_verb_struct = variable_struct_get(config, "gamepad");
        ...
        if (is_struct(_source_verb_struct))
        {
            ...
            repeat (array_length(_verb_names))
            {
                ...
                var _verb_alternate_array = variable_struct_get(_source_verb_struct, _verb_name);
                ...
                if (is_array(_verb_alternate_array))
                {
                    ...
                    repeat (array_length(_verb_alternate_array))
                    {
                        ...
                        if (is_struct(_binding))
                        {
                            with (_binding)
                            {
                                if (type == "gamepad button" && (value == 32769 || value == 32770))
