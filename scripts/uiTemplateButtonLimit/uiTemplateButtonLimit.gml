function uiTemplateButtonLimit(arg0, arg1, arg2)
{
    ...
    textXOffset = 0;
    textYOffset = 0;
    var _textElement = scribble(text).starting_format(locGetFontFromLanguage(), make_color_rgb(255, 255, 255)).msdf_border(make_color_rgb(46, 50, 59), 3).transform(textSize, textSize, 0).scale_to_box(textMaxWidth).align(1, 1);
    ...
    {
        scribble(text).starting_format(locGetFontFromLanguage(), make_color_rgb(255, 255, 255)).msdf_border(make_color_rgb(46, 50, 59), 3).transform(textSize, textSize, 0).scale_to_box(textMaxWidth).align(1, 1).blend(textBlend, 1).draw(getDrawX() + textXOffset, getDrawY() + textYOffset);
    });
}
